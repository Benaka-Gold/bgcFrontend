...
function PledgedForm() {
            ...
            if (customerResponse.status === 200) {
                ...
                if (businessResponse.status === 200) {
                    ...
                    if (businessResponse.data.pledgeCopy) {
                        const imageResponse = await getFile(businessResponse.data.pledgeCopy);
                        if (imageResponse.status === 200) {
                            setImagePreviews({ pledgeCopy: imageResponse.data.data });
    ...
    const handleFileUpload = async (e, fieldName) => {
        ...
        if (file) {
            try {
                const res = await uploadfiles(file, "customer", `${name}/pledgeCopy`);
                if (res.success) {
                    setTimeout(()=>{
        ...
        <Box
            ...
            <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate sx={{ width: { lg: "30%", sm: '70%', xs: '90%', md: '70%' }, margin: 'auto' }} >
                ...
                <Controller name="pledgeCopy" control={control}
                    ...
                    render={({ field, fieldState: { error } }) => renderFileInput(field, error, 'Pledged Copy')} />
