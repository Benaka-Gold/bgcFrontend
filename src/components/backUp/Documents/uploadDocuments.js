...
function VerificationDocuments() {
  ...
  const DeleteImagePreview = async (fieldName) => {
    ...
    if (name === "panDetails" || name === "idProof" || name === "addressProof") {
      let currentValues = getValues();
      let updated = { ...currentValues, [name]: { ...currentValues[name], file: null } };
      let response = await updateCustomer(customerId, updated);
        ...
        <Box sx={{ height: "auto", display: "flex", alignContent: "center", justifyContent: "center", }}>
          <form onSubmit={handleSubmit(onSubmit)}>
            <Box sx={{ margin: 2 }}>
              <Controller name="panDetails.number" control={control} rules={{ required: "PAN number is required" }}
                render={({ field }) => (
                  <TextField {...field} label="PAN Number" variant="outlined" margin="normal" fullWidth
                    error={!!errors.panDetails?.number} helperText={errors.panDetails?.number?.message} />)} />
              <Controller name="panDetails.file" control={control} rules={{ required: "PAN file is required" }}
                render={({ field, fieldState: { error } }) => renderFileInput(field, error, 'PAN')} />
              ...
              <Controller name="idProof.number" control={control} rules={{ required: "ID number is required" }} type='number'
                render={({ field }) => (
                  <TextField {...field} label="ID Number" variant="outlined" margin="normal" fullWidth
                    error={!!errors.idProof?.number} helperText={errors.idProof?.number?.message} />)} />

              <Controller name="idProof.file" control={control} rules={{ required: "ID proof file is required" }}
                render={({ field, fieldState: { error } }) => renderFileInput(field, error, 'ID Proof')} />
              ...
              <Controller name="addressProof.number" control={control} rules={{ required: "Address number is required" }} type="number"
                render={({ field }) => (
                  <TextField {...field} label="Address Number" variant="outlined" margin="normal" fullWidth
                    error={!!errors.addressProof?.number} helperText={errors.addressProof?.number?.message} />)} />

              <Controller name="addressProof.file" control={control} rules={{ required: "Address proof file is required" }}
                render={({ field, fieldState: { error } }) => renderFileInput(field, error, 'Address Proof')} />
